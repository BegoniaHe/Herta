...
describe("comm-channel effect (vendored)", () => {
  ...
  for (const sampleRate of [24000, 48000]) {
    for (const preset of ["terminal", "terminal_textured"]) {
      it(`${preset} @ ${sampleRate}: silence stays silent, output is finite and length-preserving, input untouched`, () => {
  ...
  it("the textured preset is deterministic, differs from the clean one, and its noise decays in silence", () => {
    const rate = 24000;
