...
describe("device-scene lighting tables (ADR 0057 §2.1b, the pale room)", () => {
  ...
  it("the light theme is a morning and the dark theme is after hours", () => {
    ...
    // The pale room keeps the anchors' fill / rim / sky at full strength and
    // the softbox at 0.32 × key (no "shaped light" damping).
  ...
  it("the morning key is the anchor's own position, above and left", () => {
    const [x, y] = lightingAt(8).position;
